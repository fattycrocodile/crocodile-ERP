    ...
    <div class="row">
        <div class="col-12 text-right">
            <a type="button" class="btn btn-info btn-min-width mr-1 mb-1"
               href="{{ route('accounting.payment.create') }}"><i
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">SUPPLIERS PAYMENT MANAGE</h4>
...
@push('scripts')
    ...
    <script>
        $('#supplier-payment-table').on('click', '.btn-delete[data-remote]', function (e) {
            e.preventDefault();
            $.ajaxSetup({
                headers: {
            ...
            if (confirm('Are you sure you want to delete this row?')) {
                $.ajax({
                    ...
                    $('#supplier-payment-table').DataTable().draw(false);
        ...
        $('#supplier-payment-table').on('click', '.btn-preview', function (e) {
            e.preventDefault();
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            ...
            var url = "{{ route('accounting.payment.voucher') }}";
            ...
            $.ajax({
                ...
                type: 'POST',
                dataType: 'json',
                data: {method: 'POST', submit: true, pr_no: pr_no}
