...
class EmployeesController extends BaseController
{
    ...
    public function joiningReport()
    {
        ...
        $religions = Lookup::items('religion');
        $marital_status = Lookup::items('marital_status');
        $this->setPageTitle('Employees Joining Report', 'Employees Joining Report');
        return view('Hr::employees.joining-report', compact('stores', 'department', 'designation', 'genders', 'religions', 'marital_status'));
    }

    public function joiningReportView(Request $request): ?JsonResponse
    {
        $response = array();
        $data = NULL;
        if ($request->has('start_date') && $request->has('end_date')) {
            ...
            $data = new Employees();
            $data = $data->where('join_date', '>=', $start_date);
            $data = $data->where('join_date', '<=', $end_date);
            if ($request->has('store_id')) {
                $store_id = $request->store_id;
                if ($store_id > 0)
                    $data = $data->where('store_id', '=', $store_id);
        ...
        $returnHTML = view('Hr::employees.joining-report-view', compact('data'))->render();
        return response()->json(array('success' => true, 'html' => $returnHTML));
    }
}
