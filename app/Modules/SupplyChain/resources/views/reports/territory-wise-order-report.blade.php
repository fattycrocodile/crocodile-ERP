            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title" id="basic-layout-form">TERRITORY WISE ORDER REPORT CRITERIA</h4>
                    ...
                    <div class="card-content collapse show">
                        <div class="card-body">
                            ...
                            <form class="form" id="mr-form" action="{{route('supply-chain.reports.territory-wise-sell-order-view')}}"
                                  method="post"
                                  autocomplete="off">
                                @csrf
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-2">
                                            <div class="form-group">
                                                ...
                                                <input type="text"
                                                       class="form-control @error('territory_code') is-invalid @enderror"
                                                       id="territory_code">
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-actions center">
                                            <a type="button" class="btn btn-warning mb-1"
                                               href="{{route('supply-chain.reports.territory-wise-sell-order')}}">
    ...
    <section class="row report-view-section" style="display: none;">
        <div class="col-sm-12">
            <div id="kick-start" class="card">
                <div class="card-header">
                    <h4 class="card-title">TERRITORY WISE ORDER REPORT</h4>
                </div>
                <div class="card-content collapse show">
                    <div class="card-body">
...
@push('scripts')
    ...
    <script type="text/javascript">
        ...
        $("#territory_name").autocomplete({
            minLength: 1,
            autoFocus: true,
            source: function (request, response) {
                var area_id = $("#area_id").val();
                $.ajax({
                    url: "{{ route('territory.name.autocomplete') }}",
                    type: 'post',
                    dataType: "json",
                    data: {
                        _token: CSRF_TOKEN,
            ...
            select: function (event, ui) {
                if (ui.item.value != '' || ui.item.value > 0) {
                    $('#territory_name').val(ui.item.name);
                    $('#territory_id').val(ui.item.value);
                    $('#territory_code').val(ui.item.code);
                } else {
                    resetTerritory()
                }
                return false;
        ...
        $("#territory_code").autocomplete({
            ...
            source: function (request, response) {
                ...
                var area_id = $("#area_id").val();
                $.ajax({
                    url: "{{ route('territory.code.autocomplete') }}",
                    type: 'post',
                    dataType: "json",
                    data: {
                        _token: CSRF_TOKEN,
        ...
        function ajaxSave() {
            ...
            $.ajax({
                url: "{{ route('supply-chain.reports.territory-wise-sell-order-view') }}",
                type: 'post',
                dataType: "json",
                cache: false,
                data: $('form').serialize(),
